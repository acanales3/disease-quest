<template>
  <div class="h-screen flex overflow-hidden">
    ...
    <!-- Right Panel — scrollable independently, no clipping -->
    <div class="w-full lg:w-1/2 h-screen overflow-y-auto px-6 sm:px-12 lg:px-20 bg-white">
      <div class="min-h-full flex flex-col justify-center py-12">
        <div class="w-full max-w-[420px] mx-auto">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <!-- Student Status -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1.5">Are you a student?</label>
              <div class="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  @click="formData.isStudent = true"
                  :class="[
                    'h-11 rounded-lg border text-sm font-medium transition-colors',
                    formData.isStudent === true
                      ? 'bg-[#4d1979] text-white border-[#4d1979]'
                      : 'bg-white text-gray-700 border-gray-300 hover:border-[#4d1979]'
                  ]"
                >
                  Yes, I'm a student
                </button>
                <button
                  type="button"
                  @click="() => { formData.isStudent = false; formData.msYear = '' }"
                  :class="[
                    'h-11 rounded-lg border text-sm font-medium transition-colors',
            ...
            <!-- MS Year (conditional) -->
            <div v-if="formData.isStudent === true">
              <label class="block text-sm font-medium text-gray-700 mb-1.5">
                MS Year <span class="text-red-500">*</span>
              </label>
              <select
                v-model="formData.msYear"
                required
                class="w-full h-11 px-3.5 bg-white border border-gray-300 rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#4d1979]/20 focus:border-[#4d1979] transition-colors"
              >
                <option value="" disabled>Select your MS year</option>
        ...
        >
          <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-8 text-center">
            ...
            <p class="text-gray-500 text-sm mb-6 leading-relaxed">
              Your account has been created successfully. You can now continue to DiseaseQuest.
            </p>
            <button
              @click="goToLogin"
...
const handleSubmit = async () => {
  ...
  if (formData.isStudent === null) {
    errorMessage.value = "Please indicate whether you are a student.";
    return;
  }

  if (formData.isStudent && !formData.msYear) {
    errorMessage.value = "Please select your MS year.";
    return;
  }
  ...
  try {
    const { data: authData, error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          ms_year: formData.isStudent ? parseInt(formData.msYear) : null,
        },
      },
    });
    ...
    // Directly update msyear in students table since the trigger won't handle it
    if (formData.isStudent && authData.user) {
      const { error: msError } = await supabase
        .from("students")
        .update({ msyear: parseInt(formData.msYear) })
