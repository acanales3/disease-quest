...
async function fetchStudents(): Promise<{
  ...
  const [studentsResult, classroomsResult] = await Promise.allSettled([
  ...
  if (classroomsResult.status === "rejected") {
    console.error("Failed to fetch classrooms:", classroomsResult.reason);
  }

  return { students, classrooms: classroomsData };
}
