  ...
  <div class="space-y-6">
    <div class="border-b border-gray-200 pb-8">
      ...
      >
        <div>
          <p
            class="text-xs font-medium text-[#4d1979] uppercase tracking-widest mb-2"
          >
            Insights
          </p>
          <h1
            class="text-3xl font-semibold text-gray-900 tracking-tight leading-snug"
          >
            Leaderboard
          </h1>
          <p class="text-gray-500 text-[15px] mt-2 leading-relaxed">
            View rankings by completed cases and average scores.
          </p>
        </div>
        <button
    ...
    <div
      v-if="top3.length > 0"
      class="flex items-end justify-center gap-3 px-8"
    >
      <!-- 2nd Place — Silver -->
      <div class="flex flex-col items-center flex-1 max-w-[220px]">
        <div class="mb-3 text-center">
          <div
            class="w-11 h-11 rounded-full flex items-center justify-center mx-auto mb-2"
            style="
              background: linear-gradient(135deg, #e8e8e8, #a8a8a8, #d4d4d4);
            "
          >
          ...
          <p class="text-xs tabular-nums mt-0.5 font-medium text-gray-400">
            {{
              top3[1]?.averageScore != null
                ? top3[1].averageScore.toFixed(2) + "%"
                : "—"
            }}
          </p>
        </div>
      ...
      <div class="flex flex-col items-center flex-1 max-w-[240px]">
        <div class="mb-3 text-center">
          <div
            ...
            style="
              background: linear-gradient(135deg, #f9e07a, #d4a017, #f5c518);
            "
          >
            <Icon
              name="lucide:crown"
          ...
          >
            {{
              top3[0]?.averageScore != null
                ? top3[0].averageScore.toFixed(2) + "%"
                : "—"
            }}
          </p>
        </div>
      ...
      <div class="flex flex-col items-center flex-1 max-w-[220px]">
        <div class="mb-3 text-center">
          <div
            class="w-11 h-11 rounded-full flex items-center justify-center mx-auto mb-2"
            style="
              background: linear-gradient(135deg, #e8b87a, #a0522d, #cd7f32);
            "
          >
          ...
          <p
            class="text-xs tabular-nums mt-0.5 font-medium"
            style="color: #a0522d"
          >
            {{
              top3[2]?.averageScore != null
                ? top3[2].averageScore.toFixed(2) + "%"
                : "—"
            }}
          </p>
        </div>
...
async function fetchClassrooms() {
  try {
    const data =
      await $fetch<{ id: number; name: string }[]>("/api/classrooms");
    if (data) {
      classrooms.value = [
        { id: -1, name: "All Classrooms" },
...
// Use adminColumns when role is admin (shows real student name column)
const columns = computed(() =>
  props.role === "admin" ? adminColumns : baseColumns,
);
