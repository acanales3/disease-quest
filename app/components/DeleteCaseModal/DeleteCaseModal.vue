...
const isMatch = computed(
  () => confirmation.value.trim().toUpperCase() === "DELETE",
);
  ...
  <Dialog :open="open" @update:open="(value) => emit('update:open', value)">
    <DialogContent class="max-w-lg">
      <DialogHeader class="text-center">
        <DialogTitle class="text-red-600 text-center">Delete Case</DialogTitle>
        <DialogDescription class="text-left">
          This will permanently delete the case and related data. This action
          cannot be undone.
        </DialogDescription>
      </DialogHeader>

      <div v-if="caseData" class="py-3">
        <div class="bg-gray-50 rounded-lg p-4 space-y-2">
          <div class="flex justify-between gap-4">
            <span class="text-sm text-gray-500">Name:</span>
            <span class="text-sm font-medium text-right">{{ caseData.name }}</span>
          </div>
          <div class="flex justify-between gap-4">
      ...
      <DialogFooter class="gap-2 sm:justify-center">
        ...
        <Button
          ...
          :disabled="!caseData || !isMatch || isBusy"
          @click="onConfirm"
        >
          {{ isBusy ? "Deleting..." : "Delete Case" }}
        </Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
