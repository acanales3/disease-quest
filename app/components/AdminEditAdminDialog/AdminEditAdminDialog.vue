...
watch(
    ...
    (val) => {
        ...
        if (!val.university) errors.value.university = "University is required";
        else if (val.university.length > 255)
            errors.value.university = "University cannot exceed 255 characters";
        else errors.value.university = "";
    },
...
<template>
    <Dialog :open="props.show" @update:open="handleOpenChange">
        <DialogContent class="max-h-[90vh] overflow-y-auto my-6">
            <DialogHeader>
                <DialogTitle
                    >Edit {{ form.first_name }}
                    {{ form.last_name }}</DialogTitle
                >
                <DialogDescription>
