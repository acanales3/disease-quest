...
export default defineEventHandler(async (event) => {
  ...
  const { data: targetInstructor, error: targetInstructorError } =
    await adminClient
      .from("instructors")
      .select("user_id")
  ...
  if (targetInstructorError)
    throw createError({
      statusCode: 500,
      message: targetInstructorError.message,
    });
  if (!targetInstructor)
    throw createError({ statusCode: 404, message: "Instructor not found" });
  ...
  if (classroomsErr)
    throw createError({ statusCode: 500, message: classroomsErr.message });
  ...
  if (classroomIds.length > 0) {
    const [{ count: eCount, error: eErr }, { count: ccCount, error: ccErr }] =
      await Promise.all([
        adminClient
    ...
    // Delete classroom_cases first (no ON DELETE CASCADE on this table)
    ...
    if (delCCErr)
      throw createError({
        statusCode: 500,
        message: `Delete classroom_cases failed: ${delCCErr.message}`,
      });
    ...
    if (delClassroomsErr)
      throw createError({
        statusCode: 500,
        message: `Delete classrooms failed: ${delClassroomsErr.message}`,
      });
  }

  // 6) Delete from Supabase Auth — cascades to public.users → public.instructors automatically
  const { error: deleteAuthError } =
    await adminClient.auth.admin.deleteUser(instructorId);
  if (
    deleteAuthError &&
